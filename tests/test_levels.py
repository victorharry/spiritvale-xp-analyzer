"""Prova de onde sai o size de cada level, e em que ordem.

Hoje a fonte e a tabela do proprio jogo (`xp_table.py`), extraida dos arquivos
...
eles que sobram se um patch mudar o arquivo e a extracao stop de valer.
"""
import sys
...
settings.save = lambda *_: None          # o teste nao toca no config real
...
def feed(app, leituras):
    for level, xp in leituras:
        app._aprender_no_level_up(Progress("Corujo", level, xp, 70, 0))


app = Fingido()

print("a tabela do jogo responde sozinha, sem nenhuma medicao do usuario:")
conferir("level 5", app._previsto("base", 5), 1620)
conferir("level 114", app._previsto("base", 114), 39_284_872)
reading = app._leitura_da_rede(Progress("Galinho", 114, 19_642_436, 70, 0))
conferir("metade do level 114 da 50%", round(reading["base_pct"], 2), 50.0)
conferir("e nao vem marcado como estimativa", reading["estimado"], False)

print("\njob no maximo vale 100%, sem consultar tabela nenhuma:")
conferir("job 70", reading["job_pct"], 100.0)
maximo = app._leitura_da_rede(Progress("Galinho", 150, 0, 70, 0))
conferir("classe 150", maximo["base_pct"], 100.0)

print("\nacima da tabela (level 162+, onde o jogo satura em 2^31):")
conferir("sem medicao, nao inventa", app._previsto("base", 200), None)

print("\nsubir de level agora VIGIA a tabela em vez de feed o config:")
quieto = Fingido()
recados = []
...
feed(quieto, [(5, 700), (5, 1200), (5, 1600), (6, 107)])
conferir("medicao que concorda nao gera notice",
         any("WARNING" in str(r) for r in recados), False)
conferir("e nao grava nada (a tabela ja sabe)",
         quieto.necessario.get("base:5"), None)

print("\nmedicao que DIVERGE da tabela vira notice, nao vira valor:")
vigia = Fingido()
avisos = []
vigia.fila = type("F", (), {"put": staticmethod(lambda x: avisos.append(x[1]))})()
# level 5 pede 1.620; fingir que so juntou 900 antes de virar e sinal de patch
feed(vigia, [(5, 900), (6, 10)])
